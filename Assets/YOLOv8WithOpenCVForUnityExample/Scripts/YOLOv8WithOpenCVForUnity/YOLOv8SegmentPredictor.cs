    ...
    public class YOLOv8SegmentPredictor
    {
        ...
        public virtual void visualize(Mat image, Mat results, bool print_results = false, bool isRGB = false)
        {
            ...
            foreach (var d in data)
            {
                ...
                float det_left = firstPersonData.x1;
                float det_top = firstPersonData.y1;
                float det_right = firstPersonData.x2;
                float det_bottom = firstPersonData.y2;

                // Check if detection's bounding box is within person's bounding box
        ...
        public virtual void visualize_mask(Mat image, Mat det, Mat masks, float alpha = 0.5f, bool isRGB = false)
        {
            ...
            cumulativeMask = Mat.zeros(image.size(), CvType.CV_8UC1);
            ...
            using (Mat masks_nxhw = masks.reshape(1, n))// [n, 160, 160] => [n, 25600]
            {
                for (int i = n - 1; i >= 0; --i)
                {
                    ...
                    det.get(i, 0, box); // Assuming detection data has bounding box information at index 0
                    float mask_left = box[0];
                    float mask_top = box[1];
                    float mask_right = box[2];
                    float mask_bottom = box[3];

                    /*
                    // Check if mask bounding box is within the specified bounding box
                    Debug.Log($"{getClassLabel(classId)} : {mask_left} <= {firstPersonData.x1 + 5} && {mask_top} <= {firstPersonData.y1 + 5} && {mask_right} >= {firstPersonData.x2 - 5} && {mask_bottom} >= {firstPersonData.y2 - 5}");
                    if (mask_left <= firstPersonData.x1 + 5 && mask_top <= firstPersonData.y1 + 5 && mask_right >= firstPersonData.x2 - 5  && mask_bottom >= firstPersonData.y2 - 5)
                    {
                        Mat m_hxw = masks_nxhw.row(i).reshape(1, h); // [1, 25600] => [160, 160]
                    ...
                    */
                    ...
                    OpenCVRect obj_rect = new OpenCVRect((int)box[0], (int)box[1], (int)(box[2] - box[0]), (int)(box[3] - box[1]));
                    OpenCVRect person_rect = new OpenCVRect((int)firstPersonData.x1, (int)firstPersonData.y1, (int)(firstPersonData.x2 - firstPersonData.x1), (int)(firstPersonData.y2 - firstPersonData.y1));

                    int x_left = Math.Max(obj_rect.x, person_rect.x);
                    int y_top = Math.Max(obj_rect.y, person_rect.y);
                    int x_right = Math.Min(obj_rect.x + obj_rect.width, person_rect.x + person_rect.width);
                    int y_bottom = Math.Min(obj_rect.y + obj_rect.height, person_rect.y + person_rect.height);

                    int intersection_width = Math.Max(0, x_right - x_left);
                    int intersection_height = Math.Max(0, y_bottom - y_top);
                    float intersection_area = intersection_width * intersection_height;
                    double obj_area = obj_rect.area();
                    float threshold = 0.5f; // Adjust threshold as needed (e.g., 0.5 for 50% overlap)

                    if (intersection_area / obj_area >= threshold)
                    {
                        Mat m_hxw = masks_nxhw.row(i).reshape(1, h); // [1, 25600] => [160, 160]
                        Mat m_hxw_roi = new Mat(m_hxw, new OpenCVRect(mask_pad_x, mask_pad_y, mask_w, mask_h));
                        Imgproc.resize(m_hxw_roi, maskMat, image.size(), -1, -1, Imgproc.INTER_LINEAR);
                        Imgproc.rectangle(colorMat, new OpenCVRect(0, 0, colorMat.width(), colorMat.height()), color, -1);
                        Core.add(maskMat, cumulativeMask, cumulativeMask);
                        Core.addWeighted(colorMat, alpha, image, alpha, 0, colorMat);
                        //colorMat.copyTo(image, maskMat);
                    }
                    //or
                    ...
                    OpenCVRect roi_rect = new OpenCVRect((int)firstPersonData.x1, (int)firstPersonData.y1, (int)(firstPersonData.x2 - firstPersonData.x1), (int)(firstPersonData.y2 - firstPersonData.y1));
