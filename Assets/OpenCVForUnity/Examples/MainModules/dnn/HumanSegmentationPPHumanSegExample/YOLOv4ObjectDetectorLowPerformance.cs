...
namespace OpenCVForUnityExample.DnnModel
{
    ...
    /// </summary>
    public class YOLOv4ObjectDetectorLowPerformance
    {
        Size input_size;
        ...
        public YOLOv4ObjectDetectorLowPerformance(string modelFilepath, string configFilepath, string classesFilepath, Size inputSize, float confThreshold = 0.25f, float nmsThreshold = 0.45f, int topK = 1000, int backend = Dnn.DNN_BACKEND_OPENCV, int target = Dnn.DNN_TARGET_CPU)
        {
            // initialize
        ...
        public virtual void visualize(Mat image, Mat results, bool print_results = false, bool isRGB = false)
        {
            ...
            // Variables to store the closest person
            ...
            for (int i = results.rows() - 1; i >= 0; --i)
            {
                ...
                // Only consider 'person' class (usually classId == 0 for person in common models, adjust if needed)
                if (classId != 0)
                    continue;
                ...
                Point boxCenter = new Point((left + right) / 2, (top + bottom) / 2);

                // Calculate the Euclidean distance from the box center to the image center
                double distance = Math.Sqrt(Math.Pow(boxCenter.x - imageCenter.x, 2) + Math.Pow(boxCenter.y - imageCenter.y, 2));

                // Update the closest person if this one is closer
            ...
            // If a closest person is found, draw their bounding box and label
            if (closestPersonIndex != -1)
            {
                float[] box = new float[4];
                results.get(closestPersonIndex, 0, box);
                float[] conf = new float[1];
                results.get(closestPersonIndex, 4, conf);
                float[] cls = new float[1];
                results.get(closestPersonIndex, 5, cls);
