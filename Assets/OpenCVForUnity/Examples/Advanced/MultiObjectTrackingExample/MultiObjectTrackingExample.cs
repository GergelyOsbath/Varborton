...
namespace OpenCVForUnityExample
{
    ...
    public class MultiObjectTrackingExample : MonoBehaviour
    {
        ...
        public UnityEvent<PositionData> _humanDetected;
        
        // Use this for initialization
        void Start()
        {
            //Debug mode enable/disable
            EnableVisualization = Config.DebugEnabled;
            if (EnableVisualization) GetComponent<MeshRenderer>().enabled = true;
            
            sourceToMatHelper = gameObject.GetComponent<WebCamTextureToMatHelper>();
        ...
        void Run()
        {
            ...
            sourceToMatHelper.outputColorFormat = WebCamTextureToMatHelper.ColorFormat.RGB; // Tracking API must handle 3 channels Mat image.
        ...
        public void OnVideoCaptureToMatHelperErrorOccurred(WebCamTextureToMatHelper.ErrorCode errorCode)
        {
            ...
            if (sourceToMatHelper.IsPlaying() && sourceToMatHelper.DidUpdateThisFrame())
            {
                ...
                else
                {
                    if (!disableObjectDetector)
                    {
                        ...
                        if (EnableVisualization) objectDetector.visualize(rgbMat, results, false, true);

                        //this part is coming from the YOLOXObjectDetector, here we figure put if a detected object is a person or not
                        
                        if (results.empty() || results.cols() < 6) return; //If there is not enough columns we don't care any more
                        bool[] detectedPersons = new bool[results.rows()];
                        for (int i = 0; i < results.rows(); i++)
                        {
                            float[] cls = new float[1];
                            results.get(i, 5, cls); //this is the class part of the matrix, other part of the code is deleted
                            detectedPersons[i] = objectDetector.ClassNames[(int)cls[0]].Equals("person"); 
                        }
                        ...
                        if (byteTracking)
                        {
                            ...
                            if (results.rows() == inputs.Count && inputs.Count == outputs.Count)
                            {
                                for (int i = 0; i < outputs.Count; i++)
                                {
                                    if (!detectedPersons[i]) continue; //if we not found a person previously, we don't care;
                                    
                                    TlwhRect rect = (TlwhRect)outputs[i].Detection.Rect;
                                    int track_id = outputs[i].TrackId;
                                    _humanDetected.Invoke(new PositionData()
                                    {
                                        PersonIndex = track_id,
                                        Position = new Vector2( (rect.Left + rect.Width * 0.5f) / texture.width, (rect.Top + rect.Height * 0.5f) / texture.height)
                                    });
                ...
                if (EnableVisualization) Utils.matToTexture2D(rgbMat, texture);
            }
        }

        private void ResetTrackers()
        {
            if (byteTracker != null) byteTracker.Clear();
        }
